...
if (!file_exists("config.php")) {
	...
	$instance = substr(md5($client_ip), 0, 8);
	$basepath = rtrim(str_replace("index.php", "", $_SERVER["SCRIPT_FILENAME"]), "/");
	// echo("base = $basepath|");

	if (!file_exists($basepath."/i/".$instance)) {
		$var1 = str_replace('index.php', '', $_SERVER['REQUEST_URI']) . "i";

		$command = "$basepath/demo/create.sh \"$basepath\" \"$basepath/i\" \"$var1\" \"$instance\"";
		...
		<div id="installer">
			...
			<?php if ($instance != "") {
				...
				<p style="padding: 10px 0;">
					Your instance has been prepared sucessfully.<br>This demo application has some security restrictions.
				</p>
				...
				<p style="padding: 10px 0; color: #a8a6a8;">
					Note: Each instance will be deleted after 30 minutes.
				</p>
				...
				?>
				<p style="padding: 10px 0;">
					<font color=red>Error</font> - Unable to generate instance.
				</p>
				<p style="padding: 10px 0; color: #a8a6a8;">
					Command: <?php echo $command; ?>
				</p>
				<p style="padding: 10px 0; color: #a8a6a8;">
					Message: <?php echo print_r($instance); ?>
				</p>
				<?php
			} ?>
		</div>
