// Creation Time: 2020-02-16 15:01:57
...
resolve()}else{if(options.fail){options.fail(status)}
reject({status:xhr.status,statusText:xhr.statusText})}}};if(options.type=='GET'){params=params?'?'+params:'';xhr.open('GET',options.url+params,!0);xhr.send(null)}else if(options.type=='POST'){xhr.open('POST',options.url,!0);xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');xhr.send(params)}})};return ajax});
...
return-1};var doRun=function(target,selector){if(['*','window','document','document.documentElement',window,document,document.documentElement].indexOf(selector)>-1)return!0;if(typeof selector!=='string'&&selector.contains){return selector===target||selector.contains(target)}
return target.closest(selector)};var eventHandler=function(event){if(!activeEvents[event.type])return;activeEvents[event.type].forEach(function(listener){if(!doRun(event.target,listener.selector))return;listener.callback(event)})};var events={on:function(types,selector,callback){if(!selector||!callback){return}
...
activeEvents[type].push({selector:selector,callback:callback})})},off:function(types,selector,callback){types.split(',').forEach(function(type){type=type.trim();if(!activeEvents[type])return;if(activeEvents[type].length<2||!selector){delete activeEvents[type];window.removeEventListener(type,eventHandler,!0);return}
var index=getIndex(activeEvents[type],selector,callback);if(index<0)return;activeEvents[type].splice(index,1)})},once:function(types,selector,callback){events.on(types,selector,function temp(event){callback(event);events.off(types,selector,temp)})},get:function(){var obj={};for(var type in activeEvents){if(activeEvents.hasOwnProperty(type)){obj[type]=activeEvents[type]}}
return obj}};let argToElement=function(selector){if(selector){if(typeof selector==='string'){const tagName=/^<(\w+)>$/.exec(selector);if(tagName!==null){return document.createElement(tagName[1])}else{return document.querySelector(selector)}}else if(selector instanceof HTMLElement){return selector}else if(selector.isOnyx){return selector.el}
throw new TypeError('Expected string | HTMLElement | OnyxJS; got '+typeof selector)}};const onyx=function(selector){let element=argToElement(selector);if(!element)return;let insertToAdjacent=(s)=>function(e){iter((j,i)=>i===0?e instanceof HTMLElement?e.insertAdjacentElement(s,j):e.element.insertAdjacentElement(s,j):element.insertAdjacentElement('afterend',j));return this};let insertAdjacent=(s)=>function(sOrE){if(typeof sOrE!=='string'){if(sOrE instanceof HTMLElement){element.insertAdjacentElement(s,sOrE)}else if('isOnyx' in sOrE){const osel=sOrE.el;element.insertAdjacentElement(s,osel);for(let i=1;i<osel.length;i++){osel.insertAdjacentElement('afterend',osel[i])}}}else{element.insertAdjacentHTML(s,sOrE)}
return this};let triggerEvent=function(event){if(!event)return;if(element){var e;if('createEvent' in document){e=document.createEvent('HTMLEvents');e.initEvent(event,!1,!0);element.dispatchEvent(e)}else{e=document.createEventObject();e.eventType=event;event.fireEvent('on'+e.eventType,e)}}};return{focus:function(){element.focus();return this},trigger:function(e){triggerEvent(e)},on:function(t,fn){events.on(t,element,fn);return this},off:function(t,fn){events.off(t,element,fn);return this},css:function(s){if(typeof s==='string'){return element.style[s]||null}else if(typeof s==='object'){const entries=Object.entries(s);for(const[key,value]of entries){element.style[key]=value}}
return this},data:function(d){if(d){element.data=d}
...
return element.innerText},addClass:function(t){element.classList.add(t);return this},removeClass:function(t){element.classList.remove(t);return this},hasClass:function(c){return element.classList.contains(c)},replaceClass:function(c,n){element.classList.remove(c);element.classList.add(n);return this},toggleClass:function(t){element.classList.toggle(t);return this},empty:function(){element.innerHTML='';return this},attr:function(k,v){if(v){element.setAttribute(k,v)}
return element.getAttribute(k)},removeAttr:function(k){element.removeAttribute(k);return this},parent:function(){return element.parentNode},children:function(){return element.childNodes},find:function(s){return onyx(element.querySelector(s))},remove:function(){element.remove();return this},before:insertAdjacent('beforebegin'),after:insertAdjacent('afterend'),first:insertAdjacent('afterbegin'),last:insertAdjacent('beforeend'),insertBefore:insertToAdjacent('beforebegin'),insertAfter:insertToAdjacent('afterend'),insertFirst:insertToAdjacent('afterbegin'),insertLast:insertToAdjacent('beforeend'),prepend:insertAdjacent('afterbegin'),append:insertAdjacent('beforeend'),getAttr:v=>element.getAttribute(v),offset:()=>element.getBoundingClientRect(),outerHeight:element.outerHeight,outerWidth:element.outerWidth,clientHeight:function(){return element.clientHeight},clientWidth:function(){return element.clientWidth},style:element.style,el:element,isOnyx:!0}};return onyx});
...
// modules/system.js
(function(global){var atheos=global.core=global.atheos=global.codiad={},amplify=global.amplify,o=global.onyx;document.addEventListener('DOMContentLoaded',function(){if(document.querySelector('#login')){global.synthetic.init()}else{atheos.confirm.init();atheos.helpers.init();atheos.modal.init();atheos.sidebars.init();atheos.storage.init();atheos.toast.init();amplify.publish('atheos.loaded',{});window.addEventListener('resize',function(){var handleWidth=10;var marginL,reduction;if(o('#sb-left').css('left')!==0&&!atheos.sidebars.settings.leftLockedVisible){marginL=handleWidth;reduction=2*handleWidth}else{marginL=o('#sb-left').clientWidth();reduction=marginL+handleWidth}
o('#editor-region').css({'margin-left':marginL+'px'});o('#editor-region').css({'margin-left':marginL+'px','height':(o('body').clientHeight())+'px'});o('#root-editor-wrapper').css({'height':(o('body').clientHeight()-57)+'px'});if(atheos.editor){atheos.editor.resize();atheos.active.updateTabDropdownVisibility()}})}})})(this);(function(global){var atheos=global.atheos,$=global.jQuery;$.loadScript=function(url,arg1,arg2){console.warn('$.loadScript is depreciated, please use "atheos.helpers.loadScript"');atheos.helpers.loadScript(url,arg1,arg2)};$.ctrl=function(key,callback,args){console.warn('$.ctrl is depreciated, please use "atheos.keybind.bind"');atheos.keybind.bind(key,callback,args)};amplify.subscribe('context-menu.onShow',function(){console.warn('[Deprecation] context-menu amplify event: please subscribe to contextMenu')})})(this);
// modules/chronometer.js
(function(global){var atheos=global.atheos,amplify=global.amplify;amplify.subscribe('atheos.loaded',function(){atheos.chrono.init()});atheos.chrono={kilo:'',mega:'',giga:'',init:function(){console.log('Chronometer Initialized');var chrono=this;chrono.kilo=setInterval(function(){amplify.publish('chrono.kilo')},1000);chrono.mega=setInterval(function(){amplify.publish('chrono.mega')},10000);chrono.giga=setInterval(function(){amplify.publish('chrono.giga')},100000)},}}(this));
// modules/confirm.js
(function(global){'use strict';var atheos=global.atheos,i18n=global.i18n;atheos.confirm={icons:{'error':'exclamation-circle','notice':'info-circle','success':'check-circle','warning':'exclamation-triangle'},init:function(){console.log('Confirmation Initialized')},createConfirm:function(text,type){var overlay=document.createElement('div'),dialog=document.createElement('div'),actions=document.createElement('div');overlay.id='confirm-overlay';dialog.id='confirm-dialog';actions.classList.add('actions');dialog.appendChild(document.createElement('h2'));dialog.appendChild(actions);overlay.appendChild(dialog);document.body.appendChild(overlay);return overlay},showConfirm:function(options){if(options&&typeof options==='object'){var overlay=document.querySelector('#confirm-overlay')||this.createConfirm(),dialog=overlay.querySelector('#confirm-dialog'),message=dialog.querySelector('h2'),actions=dialog.querySelector('.actions');overlay.style.display='block';if(options.message){message.innerText=i18n(options.message)}
var confirm=document.createElement('button');var deny=document.createElement('button');confirm.innerText=options.confirm.message;deny.innerText=options.deny.message;confirm.addEventListener('click',function(){options.confirm.fnc();overlay.style.display='none'});deny.addEventListener('click',function(){options.deny.fnc();overlay.style.display='none'});actions.innerHTML='';actions.appendChild(confirm);actions.appendChild(deny)}}}})(this);
// modules/helpers.js
var log=function(m,t){if(t){console.trace(m)}else{console.log(m)}};(function(global){'use strict';var atheos=global.atheos,ajax=global.ajax;atheos.helpers={icons:{},settings:{},init:function(options){console.log('Helpers Initialized')},file:{getShortName:function(path){return path.split('/').pop()},getExtension:function(path){return path.split('.').pop()},},extend:function(obj,src){for(var key in src){if(src.hasOwnProperty(key))obj[key]=src[key]}
return obj},trigger:function(selector,event){console.warn('Trigger Helper will be depreciated on next release');if(!event||!selector)return;var element;if(selector.self==window){element=selector}else{element=selector.nodeType===Node.ELEMENT_NODE?selector:document.querySelector(selector)}
if(element){var e;if('createEvent' in document){e=document.createEvent('HTMLEvents');e.initEvent(event,!1,!0);element.dispatchEvent(e)}else{e=document.createEventObject();e.eventType=event;el.fireEvent('on'+e.eventType,e)}}},loadScript:function(url,arg1,arg2){var cache=!0,callback=null;if(typeof arg1==='function'){callback=arg1;cache=arg2||cache}else{cache=arg1||cache;callback=arg2||callback}
...
// modules/keybind.js
(function(global){var atheos=global.atheos,amplify=global.amplify;amplify.subscribe('atheos.loaded',function(){atheos.keybind.init()});atheos.keybind={init:function(){document.addEventListener('keyup',function(e){if(e.keyCode===27){atheos.modal.unload()}});this.bind(83,function(){atheos.active.save()});this.bind(79,function(){atheos.active.openInBrowser()});this.bind(70,function(){atheos.editor.openSearch('find')});this.bind(82,function(){atheos.editor.openSearch('replace')});this.bind(38,function(){atheos.active.move('up')});this.bind(40,function(){atheos.active.move('down')});this.bind(71,function(){if(atheos.finder){atheos.finder.expandFinder()}})},bind:function(key,callback,args){document.addEventListener('keydown',function(e){if(e.keyCode===key&&(e.ctrlKey||e.metaKey)){args=args||[];if(!(e.ctrlKey&&e.altKey)){e.preventDefault();callback.apply(this,args);return!1}}})}}})(this);
// modules/modal.js
(function(global,$){'use strict';var atheos=global.atheos,ajax=global.ajax,amplify=global.amplify,o=global.onyx;atheos.modal={settings:{isModalVisible:!1},init:function(){console.log('Modal Initialized')},createModal:function(){var modal=atheos.modal;var overlay=o('<div>'),wrapper=o('<div>'),content=o('<div>'),drag=o('<i>'),close=o('<i>');overlay.attr('id','modal_overlay');overlay.on('click',function(event){if(event.target.id!=='modal_overlay'){return}
modal.unload()},!1);wrapper.attr('id','modal_wrapper');content.attr('id','modal_content');close.addClass('icon-cancel');close.on('click',modal.unload);drag.addClass('icon-arrows');drag.on('mousedown',function(){drag.addClass('active');modal.drag(wrapper)},!1);wrapper.append(close);wrapper.append(drag);wrapper.append(content);document.body.appendChild(wrapper.el);document.body.appendChild(overlay.el);return wrapper},load:function(width,url,data){data=data||{};width=width>400?width:400;var wrapper=o('#modal_wrapper')||this.createModal(),content=o('#modal_content');$('#modal_content form').die('submit');wrapper.css({'top':'15%','left':'calc(50% - '+(width/2)+'px)','min-width':width+'px'});content.html('<div id="modal_loading"></div>');this.loadProcess=ajax({url:url,data:data,success:function(data){$('#modal_content').html(data);var input=wrapper.find('input[autofocus="autofocus"]');if(input){input.focus()}}});amplify.publish('modal.onLoad',{animationPerformed:!1});wrapper.css({'display':'block'});o('#modal_overlay').css({'display':'block'});this.settings.isModalVisible=!0},hideOverlay:function(){o('#modal_overlay').style.display='none'},hide:function(){var wrapper=o('#modal_wrapper'),overlay=o('#modal_overlay');wrapper.removeClass('modal-active');overlay.removeClass('modal-active');wrapper.on('transitionend',function(){wrapper.remove();overlay.remove()});atheos.editor.focus();this.settings.isModalVisible=!1},unload:function(){amplify.publish('modal.onUnload',{animationPerformed:!1});o('#modal_overlay').css({'display':''});o('#modal_wrapper').css({'display':''});o('#modal_content').empty();atheos.modal.settings.isModalVisible=!1;atheos.editor.focus()},drag:function(wrapper){var rect=wrapper.offset(),mouseX=window.event.clientX,mouseY=window.event.clientY,modalX=rect.left,modalY=rect.top;function moveElement(event){if(wrapper!==null){wrapper.style.left=modalX+event.clientX-mouseX+'px';wrapper.style.top=modalY+event.clientY-mouseY+'px'}}
function removeListeners(){document.removeEventListener('mousemove',moveElement,!1);document.removeEventListener('mouseup',removeListeners,!1);o('.icon-arrows.active').removeClass('active')}
document.addEventListener('mousemove',moveElement,!1);document.addEventListener('mouseup',removeListeners,!1)}}})(this,jQuery);
// modules/sidebars.js
(function(global){'use strict';var atheos=global.atheos,amplify=global.amplify,o=global.onyx,hoverintent=global.hoverintent;atheos.sidebars={settings:{leftLockedVisible:!0,rightLockedVisible:!1,isLeftSidebarOpen:!0,isRightSidebarOpen:!1,leftSidebarClickOpen:!1,rightSidebarClickOpen:!1},init:function(){amplify.subscribe('settings.loaded',function(settings){var sidebars=atheos.sidebars,sbLeftWidth=atheos.storage.get('sidebars.sb-left-width'),sbRightWidth=atheos.storage.get('sidebars.sb-right-width');if(sbLeftWidth!==null){o('#sb-left').css({'width':sbLeftWidth})}
