...
// modules/i18n.js
// modules/i18n.js
var i18n=(function(lang){return function(word,args){var x;var returnw=(word in lang)?lang[word]:word;for(x in args){returnw=returnw.replace("%{"+x+"}%",args[x])}
return returnw}})([]);
// modules/onyx.js
// modules/onyx.js
(function(root,factory){if(typeof define==='function'&&define.amd){define([],function(){return factory(root)})}else if(typeof exports==='object'){module.exports=factory(root)}else{root.onyx=factory(root)}})(typeof global!=='undefined'?global:typeof window!=='undefined'?window:this,function(window){'use strict';var activeEvents={};var getIndex=function(arr,selector,callback){for(var i=0;i<arr.length;i++){if(arr[i].selector===selector){if(!callback){return i}else if(arr[i].callback.toString()===callback.toString()){return i}}}
...
activeEvents[type].splice(index,1)})},once:function(types,selector,callback){events.on(types,selector,function temp(event){callback(event);events.off(types,selector,temp)})}};window.events={list:function(){var obj={};for(var type in activeEvents){if(activeEvents.hasOwnProperty(type)){obj[type]=activeEvents[type]}}
...
let insertToAdjacent=(s)=>function(e){if(e instanceof HTMLElement){e.insertAdjacentElement(s,element)}else{e.element.insertAdjacentElement(s,element)}};let insertAdjacent=(s)=>function(sOrE){if(typeof sOrE!=='string'){if(sOrE instanceof HTMLElement){element.insertAdjacentElement(s,sOrE)}else if('isOnyx' in sOrE){const osel=sOrE.el;element.insertAdjacentElement(s,osel);for(let i=1;i<osel.length;i++){osel.insertAdjacentElement('afterend',osel[i])}}}else{element.insertAdjacentHTML(s,sOrE)}
return this};let triggerEvent=function(event){if(element&&event){var e;if('createEvent' in document){e=document.createEvent('HTMLEvents');e.initEvent(event,!1,!0);element.dispatchEvent(e)}else{e=document.createEventObject();e.eventType=event;event.fireEvent('on'+e.eventType,e)}}};return{focus:()=>element.focus(),show:function(){element.style.display='block'},hide:function(){element.style.display='none'},trigger:function(e){triggerEvent(e)},once:function(t,fn){events.once(t,element,fn)},on:function(t,fn){events.on(t,element,fn)},off:function(t,fn){events.off(t,element,fn)},css:function(a,v){if(typeof a==='string'){if(v){element.style[a]=v}
return element.style[a]||null}else if(typeof a==='object'){const entries=Object.entries(a);for(const[key,value]of entries){element.style[key]=value}}},data:(d)=>{if(d)element.data=d;return element.data},html:(h)=>{if(h)element.innerHTML=h;return element.innerHTML},text:(t)=>{if(t)element.innerText=t;return element.innerText},value:(v)=>{if(v)element.value=v;return element.value},addClass:function(c){element.classList.add(...c.split(' '))},removeClass:function(c){if(c){element.classList.remove(...c.split(' '))}else{element.className=''}},hasClass:function(c){return element.classList.contains(c)},replaceClass:function(c,n){this.removeClass(c);this.addClass(n)},toggleClass:function(t){return element.classList.toggle(t)},empty:()=>element.innerHTML='',exists:function(){return(element&&element.nodeType)},attr:function(k,v){if(v){element.setAttribute(k,v)}
return element.getAttribute(k)},removeAttr:(k)=>element.removeAttribute(k),parent:()=>onyx(element.parentNode),siblings:function(s){var siblings=[];var sibling=element.parentNode.firstElementChild;do{if(!s||sibling.matches(s)){siblings.push(onyx(sibling))}
...
// modules/types.js
// modules/types.js
(function(root,factory){if(typeof define==='function'&&define.amd){define([],function(){return factory(root)})}else if(typeof exports==='object'){module.exports=factory(root)}else{root.types=factory(root)}})(typeof global!=='undefined'?global:typeof window!=='undefined'?window:this,function(window){'use strict';const types={isString:function(value){return typeof value==='string'||value instanceof String},isNumber:function(value){return typeof value==='number'&&isFinite(value)},isArray:function(value){return value&&typeof value==='object'&&value.constructor===Array},isFunction:function(value){return typeof value==='function'},isObject:function(value){return value&&typeof value==='object'&&value.constructor===Object},isNull:function(value){return value===null},isBoolean:function(value){return typeof value==='boolean'},isRegExp:function(value){return value&&typeof value==='object'&&value.constructor===RegExp},isError:function(value){return value instanceof Error&&typeof value.message!=='undefined'},isDate:function(value){return value instanceof Date},isSymbol:function(value){return typeof value==='symbol'}};return types});
// modules/system.js
// modules/system.js
(function(global){var atheos=global.atheos={},amplify=global.amplify;document.addEventListener('DOMContentLoaded',function(){if(document.querySelector('#login')||document.querySelector('#installer')){global.synthetic.init()}else{atheos.chrono.init();atheos.keybind.init();atheos.codiad.init()}
amplify.publish('atheos.loaded');amplify.publish('atheos.plugins')})})(this);
// modules/alert.js
// modules/alert.js
(function(global){'use strict';var atheos=global.atheos,i18n=global.i18n,oX=global.onyx;atheos.alert={create:function(text,type){var element=oX('<div>');element.attr('id','alert');document.body.appendChild(element.el);return element},show:function(options){if(options&&typeof options==='object'){var alert=this;var overlay=oX('#overlay')||atheos.common.createOverlay(),element=oX('#alert')||this.create();overlay.show();element.show();if(options.banner){element.append(document.createElement('h1'));element.find('h1')[0].text(i18n(options.banner))}
if(options.message){element.append(document.createElement('h2'));element.find('h2')[0].text(i18n(options.message))}
if(options.data){element.append(document.createElement('pre'));element.find('pre')[0].text(i18n(options.data))}
if(options.actions||(options.positive&&options.negative)){var actions=oX('<div>');actions.addClass('actions');element.append(actions);if(options.actions){options.actions.forEach(function(action){let button=document.createElement('button');button.innerText=i18n(action.message);button.addEventListener('click',function(){action.fnc();alert.hide()});actions.append(button)})}else{var positive=document.createElement('button');var negative=document.createElement('button');positive.innerText=i18n(options.positive.message);negative.innerText=i18n(options.negative.message);positive.addEventListener('click',function(){options.positive.fnc();alert.hide()});negative.addEventListener('click',function(){options.negative.fnc();alert.hide()});actions.append(positive);actions.append(negative)}}}},hide:function(){var overlay=oX('#overlay');var element=oX('#alert');if(overlay){overlay.hide()}
if(element){element.empty();element.hide()}}}})(this);
// modules/animation.js
// modules/animation.js
(function(global){'use strict';var atheos=global.atheos;atheos.animation={slide:function(direction,target,duration=500){target.style.overflow='hidden';target.style.transitionProperty='height, margin, padding';target.style.transitionDuration=duration+'ms';var zeroStyles=function(){target.style.height=0;target.style.paddingTop=0;target.style.paddingBottom=0;target.style.marginTop=0;target.style.marginBottom=0};if(window.getComputedStyle(target).display==='none'||direction==='open'){target.style.removeProperty('display');var display=window.getComputedStyle(target).display;display=display==='none'?'block':display;target.style.display=display;var height=target.offsetHeight;zeroStyles();target.offsetHeight;target.style.boxSizing='border-box';target.style.height=height+'px';target.style.removeProperty('padding-top');target.style.removeProperty('padding-bottom');target.style.removeProperty('margin-top');target.style.removeProperty('margin-bottom');window.setTimeout(()=>{target.setAttribute('style','')},duration)}else{target.style.boxSizing='border-box';target.style.height=target.offsetHeight+'px';target.offsetHeight;zeroStyles();window.setTimeout(()=>{target.setAttribute('style','');target.style.display='none'},duration)}}}})(this);
// modules/chrono.js
// modules/chrono.js
(function(global){var atheos=global.atheos,amplify=global.amplify;atheos.chrono={kilo:'',mega:'',giga:'',init:function(){var chrono=this;chrono.kilo=setInterval(function(){amplify.publish('chrono.kilo')},1000);chrono.mega=setInterval(function(){amplify.publish('chrono.mega')},10000);chrono.giga=setInterval(function(){amplify.publish('chrono.giga')},100000)},}}(this));
// modules/codiad.js
// modules/codiad.js
(function(global){var atheos=global.atheos,amplify=global.amplify,$=global.jQuery;global.codiad=global.atheos;atheos.codiad={init:function(){global.codiad.message=global.atheos.toast;atheos.filemanager.getShortName=function(path){return atheos.common.getNodeName(path)};atheos.filemanager.getExtension=function(path){return atheos.common.getNodeExtension(path)};atheos.filemanager.getType=function(path){return atheos.common.getNodeType(path)}}};$.loadScript=function(url,arg1,arg2){atheos.common.loadScript(url,arg1,arg2)};$.ctrl=function(key,callback,args){atheos.keybind.bind(key,callback,args)};amplify.subscribe('contextmenu.show',function(obj){amplify.publish('context-menu.onShow',obj)});amplify.subscribe('contextmenu.hide',function(obj){amplify.publish('context-menu.onHide',obj)})})(this);
// modules/common.js
// modules/common.js
const log=function(m,t){try{const json=JSON.parse(m);const type=Object.prototype.toString.call(json);if(type==='[object Object]'||type==='[object Array]'){m=json}}finally{if(t){console.trace(m)}else{console.log(m)}}};(function(global){'use strict';var atheos=global.atheos,ajax=global.ajax,o=global.onyx;atheos.common={getNodeName:function(path){return path.split('/').pop()},getNodeExtension:function(path){return path.split('.').pop()},getNodeType:function(path){return o('#file-manager a[data-path="'+path+'"]').attr('data-type')},_basename:function(path,suffix){var b=path;var lastChar=b.charAt(b.length-1);if(lastChar==='/'||lastChar==='\\'){b=b.slice(0,-1)}
b=b.replace(/^.*[\/\\]/g,'');if(typeof suffix==='string'&&b.substr(b.length-suffix.length)==suffix){b=b.substr(0,b.length-suffix.length)}
return b},_dirname:function(path){return path.replace(/\\/g,'/').replace(/\/[^\/]*\/?$/,'')},extend:function(obj,src){for(var key in src){if(src.hasOwnProperty(key)){obj[key]=src[key]}}
return obj},serializeForm:function(form){var field,l,s=[];var o={};if(typeof form==='object'&&form.nodeName==='FORM'){var len=form.elements.length;for(var i=0;i<len;i++){field=form.elements[i];if(!field.name||field.disabled||field.nodeName==='BUTTON'||['file','reset','submit','button'].indexOf(field.type)>-1){continue}
if(field.type==='select-multiple'){l=form.elements[i].options.length;for(var j=0;j<l;j++){if(field.options[j].selected){o[field.name]=field.options[j].value}}}else if(field.type!=='checkbox'&&field.type!=='radio'){o[field.name]=field.value}else if(field.checked){if(o[field.name]){o[field.name].push(field.value)}else{o[field.name]=[field.value]}}}}
...
// modules/keybind.js
// modules/keybind.js
(function(global){var atheos=global.atheos,amplify=global.amplify;atheos.keybind={init:function(){this.bind(27,function(){atheos.modal.unload()});this.bind(83,function(){atheos.active.save()});this.bind(79,function(){atheos.active.openInBrowser()});this.bind(70,function(){atheos.editor.openSearch('find')});this.bind(82,function(){atheos.editor.openSearch('replace')});this.bind(38,function(){atheos.active.move('up')});this.bind(40,function(){atheos.active.move('down')});this.bind(71,function(){if(atheos.finder){atheos.finder.expandFinder()}})},bind:function(key,callback,args){document.addEventListener('keydown',function(e){if(e.keyCode===key&&(e.ctrlKey||e.metaKey)){args=args||[];if(!(e.ctrlKey&&e.altKey)){e.preventDefault();callback.apply(this,args);return!1}}})}}})(this);
// modules/modal.js
// modules/modal.js
(function(global,$){'use strict';var atheos=global.atheos,ajax=global.ajax,amplify=global.amplify,oX=global.onyx;atheos.modal={settings:{isModalVisible:!1},create:function(){var modal=atheos.modal;var wrapper=oX('<div>'),content=oX('<div>'),drag=oX('<i>'),close=oX('<i>');wrapper.attr('id','modal_wrapper');content.attr('id','modal_content');close.addClass('close fas fa-times-circle');close.on('click',atheos.modal.unload);drag.addClass('drag fas fa-arrows-alt');drag.on('mousedown',function(){drag.addClass('active');modal.drag(wrapper)},!1);wrapper.append(close);wrapper.append(drag);wrapper.append(content);document.body.appendChild(wrapper.el);return wrapper},load:function(width,url,data){data=data||{};width=width>400?width:400;var overlay=oX('#overlay')||atheos.common.createOverlay(),wrapper=oX('#modal_wrapper')||this.create(),content=oX('#modal_content');$('#modal_content form').die('submit');wrapper.css({'top':'15%','left':'calc(50% - '+(width/2)+'px)','min-width':width+'px'});content.html('<div id="modal_loading"></div>');this.ready=ajax({url:url,data:data,success:function(data){$('#modal_content').html(data);var input=wrapper.find('input[autofocus="autofocus"]')[0];if(input){input.focus()}
amplify.publish('modal.loaded')}});wrapper.show();oX('#overlay').show();this.settings.isModalVisible=!0},resize:function(){var wrapper=oX('#modal_wrapper');if(wrapper){var width=wrapper.clientWidth();width=width>1000?1000:width;wrapper.css({'top':'15%','left':'calc(50% - '+(width/2)+'px)','min-width':width+'px'})}},hideOverlay:function(){oX('#overlay').hide()},unload:function(){amplify.publish('modal.unload');oX('#modal_content').off('submit');oX('#overlay').hide();oX('#modal_wrapper').hide();oX('#modal_content').empty();atheos.modal.settings.isModalVisible=!1;atheos.editor.focus()},drag:function(wrapper){var element=wrapper.el;var rect=wrapper.offset(),mouseX=window.event.clientX,mouseY=window.event.clientY,modalX=rect.left,modalY=rect.top;function moveElement(event){if(element){element.style.left=modalX+event.clientX-mouseX+'px';element.style.top=modalY+event.clientY-mouseY+'px'}}
function disableSelect(e){e.preventDefault()}
function removeListeners(){document.removeEventListener('mousemove',moveElement,!1);document.removeEventListener('mouseup',removeListeners,!1);window.removeEventListener('selectstart',disableSelect);oX('#modal_wrapper .drag').removeClass('active')}
