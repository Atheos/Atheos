...
trait Helpers {
    ...
    public static function log($text, $name = "global") {
        ...
        // Load current log (unwrapped content)
        $existing = Common::load($file, $namespace);
        $lines = $existing ? explode(PHP_EOL, trim($existing)) : [];
        ...
        // Save raw log
        Common::save($file, implode(PHP_EOL, $lines), $namespace);
    }
